...
$(BUILD_DIR)/%: $(OBJ_DIR)/$(APP_DIR)/%.o $(OBJS)
	-@$(MKDIR) $(dir $@)
ifdef VERBOSE
	$(CXX) $(addprefix -L,$(LIBRARIES)) $^ $(LDFLAGS) -lnvToolsExt -o $@
else
	@echo linking $@
	@$(CXX) $(addprefix -L,$(LIBRARIES)) $^ $(LDFLAGS) -lnvToolsExt -o $@
endif
...
$(LIB_DIR)/libclaire.so: $(CPU_OBJS) $(GPU_OBJS)
	-@$(MKDIR) $(dir $@)
	@echo "export LD_LIBRARY_PATH=$(abspath $(LIB_DIR)):\$$LD_LIBRARY_PATH" > env_source.sh
ifdef VERBOSE
	$(CXX) $(addprefix -L,$(LIBRARIES)) $^ $(subst -lclaire,,$(LDFLAGS)) -lnvToolsExt -shared -o $@
else
	@echo linking $@
	@$(CXX) $(addprefix -L,$(LIBRARIES)) $^ $(subst -lclaire,,$(LDFLAGS)) -lnvToolsExt -shared -o $@
endif
