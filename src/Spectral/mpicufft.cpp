...
template<typename T> struct cuFFT {
  ...
  static decltype(cufftXtExec)* execXtR2C;
  static decltype(cufftXtExec)* execXtC2R;
...
template<typename T> decltype(cufftXtExec)* cuFFT<T>::execXtR2C = cufftXtExec;
template<typename T> decltype(cufftXtExec)* cuFFT<T>::execXtC2R = cufftXtExec; */
...
template<typename T> MPIcuFFT<T>::MPIcuFFT(MPI_Comm comm, bool cuda_aware)
  ...
  //v.k: MPI on One Node with Multi-GPU
  pidx = 1;
...
template<typename T> MPIcuFFT<T>::~MPIcuFFT() {
  ...
  ///
   //if (plan_r2c) cudaCheck(cufftDestroy(plan_r2c));
   //if (plan_c2r) cudaCheck(cufftDestroy(plan_c2r));
...
template<typename T> void MPIcuFFT<T>::initFFT(size_t nx, size_t ny, size_t nz, bool allocate) {
  ...
  ////
  //cudaCheck(cufftCreate(&plan_r2c));
  //cudaCheck(cufftCreate(&plan_c2r));

  cudaCheck(cufftSetAutoAllocation(planR2C, 0));
  cudaCheck(cufftSetAutoAllocation(planC2R, 0));

  //cudaCheck(cufftSetAutoAllocation(plan_r2c, 0));
  //cudaCheck(cufftSetAutoAllocation(plan_c2r, 0));
  
  if (fft3d) { // combined 3d fft
    // v.k: single transform --> batch = 1
    ...
    //v.k. Executiontype (last parameter) needs to match precision and be of a complex type!
    //long long  n[] = {isizex[pidx], isizey, isizez};
    //cudaCheck(cufftXtMakePlanMany(plan_r2c, 3, n , , 1, 1, CUDA_R_16F, 0, 1, 1, CUDA_C_16F, 1, &ws_r2c, CUDA_C_16F));
    //cudaCheck(cufftXtMakePlanMany(plan_c2r, 3, n , NULL, 1, 1, CUDA_C_16F, NULL, 1, 1, CUDA_R_16F, 1, &ws_c2r, CUDA_C_16F));
    
    printf("FFT3D/single GPU/single transform/ single Precision");

    fft_worksize = std::max(ws_r2c, ws_c2r);
  } 
    else { // 2d slab decomposition fft
    
    // v.k : e.x.  fft = {256, 300, 256} --> batch = 128 ?
...
template<typename T> void MPIcuFFT<T>::setWorkArea(void *device, void *host) {
  ...
  ////
    //  cudaCheck(cufftSetWorkArea(plan_r2c, mem_d[0]));
     // cudaCheck(cufftSetWorkArea(plan_c2r, mem_d[0]));
  }
  else {
...
//v.k.
/*template<typename T> void MPIcuFFT<T>::execXtR2C(void *out, const void *in) {
  if (!initialized) return;
  using R_t =  typename cuFFT<T>::R_t;
  using C_t =  typename cuFFT<T>::C_t;
  R_t *real    = cuFFT<T>::real(in);
  ...
  cudaMalloc(&d_data, sizeof(ftype)* worksize_d);

  if (fft3d) {
    cuFFT<T>::execXtR2C(plan_r2c, d_data, d_data, CUFFT_FORWARD);
    cudaCheck(cudaDeviceSynchronize());
  }
}
template<typename T> void MPIcuFFT<T>::execXtC2R(void *out, const void *in) {
  if (!initialized) return;
  using R_t = typename cuFFT<T>::R_t;
  using C_t = typename cuFFT<T>::C_t;
  R_t *real    = cuFFT<T>::real(out);
  ...
  if (fft3d) {
    cuFFT<T>::execXtC2R(plan_c2r, d_data, d_data, CUFFT_INVERSE);
    cudaCheck(cudaDeviceSynchronize());
  }
}*/
